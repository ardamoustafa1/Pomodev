...
function actuallyStartTimer() {
    ...
    // 250ms aralıkla güncelle - setInterval(1000) tarayıcı throttling'de 2sn atlamaya yol açıyordu
    timer = setInterval(timerTick, 250);
...
function handleVisibilityChange() {
    if (document.hidden) {
        ...
        if (isRunning && backgroundStartTime > 0) {
            ...
            // Süre bittiyse tamamlanma mantığını çalıştır (otomatik mola/pomodoro dahil)
            if (remainingTime <= 0) {
                if (timer) clearInterval(timer);
                timer = null;
                isRunning = false;
                ...
                tickAudio.pause();
                if (tickAudio.src) tickAudio.src = '';
